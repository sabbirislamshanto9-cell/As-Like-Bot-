...
async def _send_page_message(message, text, reply_markup=None, entities=None):
    ...
    # Keep the main menu visible in private chats after every page click.
    # The zero-width message is intentionally invisible to the user.
    if getattr(getattr(message, "chat", None), "type", None) == "private":
        try:
            ...
            logger.exception("Could not refresh persistent main menu for page")
    return sent
...
def make_inline_button(text, callback_data=None, style=None, url=None,
                       icon_custom_emoji_id=PREMIUM_BUTTON_ICON_ID):
    """Build an inline/page button with a native Premium/custom icon by default."""
    text = strip_premium_emoji_tags(text)
    kwargs = {}
