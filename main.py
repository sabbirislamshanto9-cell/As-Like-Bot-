...
def get_payment_presets():
    """Return enabled fixed Add Money amounts; custom amount is always separate."""
    settings = load_data("settings")
    saved = settings.get("payment_presets") if isinstance(settings, dict) else None
    if isinstance(saved, list):
        ...
        for value in saved:
            try:
                ...
                continue
            if amount in BOHUDUR_PAYMENT_PRESETS and amount not in enabled:
                enabled.append(amount)
        return tuple(enabled)
    return BOHUDUR_PAYMENT_PRESETS


def set_payment_presets(presets):
    settings = load_data("settings")
    ...
    for value in presets:
        try:
            ...
            continue
        if amount in BOHUDUR_PAYMENT_PRESETS and amount not in clean:
            clean.append(amount)
    settings["payment_presets"] = clean
    return save_data("settings", settings)


def remove_payment_preset(amount):
    try:
        amount = int(amount)
    except (TypeError, ValueError):
        return False
    if amount not in BOHUDUR_PAYMENT_PRESETS:
        return False
    return set_payment_presets([x for x in get_payment_presets() if x != amount])
...
MENU_LABEL_DEFAULTS = {
    "balance": "𝐁𝐀𝐋𝐀𝐍𝐂𝐄",
    "add_money": "𝐀𝐃𝐃 𝐌𝐎𝐍𝐄𝐘",
    "like_history": "𝐋𝐈𝐊𝐄 𝐔𝐒𝐄 𝐇𝐈𝐒𝐓𝐎𝐑𝐘",
    "order_history": "𝐎𝐑𝐃𝐄𝐑 𝐇𝐈𝐒𝐓𝐎𝐑𝐘",
    "auto_package": "𝐀𝐔𝐓𝐎 𝐏𝐀𝐂𝐊𝐀𝐆𝐄",
    "bonus_220": "𝟐𝟎𝟎 𝐋𝐈𝐊𝐄",
...
def add_premium_page_emoji(text, entities=None):
    """Do not add the shared Premium/custom emoji to page content.

    Custom icons are intentionally limited to the persistent main reply
    keyboard. Page/submenu text and inline controls stay clean.
    """
    return str(text or ""), list(entities or []), False


async def _send_page_message(message, text, reply_markup=None, entities=None):
    """Send page controls and keep the normal main menu visible.

    Telegram messages cannot carry both a ReplyKeyboardMarkup and an
    InlineKeyboardMarkup at the same time. Therefore page controls stay on
    the page message, while a tiny separate message refreshes the persistent
    main reply keyboard underneath.
    """
    kwargs = {}
    if entities:
    ...
    sent = await message.reply_text(str(text), **kwargs)

    # Do NOT send a second invisible/zero-width message here. Telegram keeps
    # the existing ReplyKeyboardMarkup visible in the chat until it is
    # explicitly removed, so refreshing it would create a blank duplicate
    # message after every button click.
    return sent


async def reply_editable_content(message, key, reply_markup=None, **values):
    """Send editable content without adding the extra Premium emoji to page text.

    Premium/custom emoji IDs remain enabled on keyboard buttons only.
    """
    text, entities, custom = render_content_entities(key, **values)
    if custom and entities:
        return await _send_page_message(message, text, reply_markup=reply_markup, entities=entities)
    return await _send_page_message(message, format_bold(text), reply_markup=reply_markup)
...
def can_use_like(user_id, target_uid=None):
    """Allow exactly one successful free-like request per Telegram user per BD day."""
...
def make_inline_button(text, callback_data=None, style=None, url=None,
    ...
    """Build a clean inline/page button without the shared Premium icon.

    The Premium/custom emoji is intentionally reserved for the persistent
    main reply keyboard; submenu buttons such as amount, package, back,
    cancel, confirm, join and direct-link buttons have no custom icon.
    """
    text = strip_premium_emoji_tags(text)
    kwargs = {}
...
def build_add_money_keyboard():
    rows = []
    for amount in get_payment_presets():
        rows.append([
            make_inline_button(
...
async def menu_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if menu_is("balance"):
        ...
        page_text = "📜 ORDER HISTORY — SELECT A PAGE"
        page_entities = []
        await _send_page_message(
            update.message,
            page_text,
        ...
        if claimed:
            ...
            keyboard = InlineKeyboardMarkup([
                [make_inline_button("📢 JOIN REQUIRED CHANNEL", url=REQUIRED_CHANNELS[0]["link"], style="primary")],
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await reply_editable_content(update.message, key, emoji=emoji)
...
async def like_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Every user in an authorized group, including group administrators, gets
    # exactly one successful /like request per Bangladesh day.
    if chat.type in ["group", "supergroup"]:
        if not can_use_like(user.id):
            await update.message.reply_text(
                format_bold(
                    "❌ আপনি আজকের Free Like ইতিমধ্যে নিয়েছেন।\n\n"
                    "🛒 আরও Like-এর জন্য Auto Like Package কিনুন।\n"
                    "নিচের bot-এ Order করুন: @Asfflikebot\n\n"
                    "⏰ ভোর ৪:০০টায় আবার ১টি Free Like request নিতে পারবেন."
                ),
                reply_markup=InlineKeyboardMarkup([[
                    make_inline_button("🛒 AUTO LIKE PACKAGE", url="https://t.me/Asfflikebot", style="success")
                ]]),
            )
            return
...
async def removepayment_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin command to hide a fixed Add Money amount from the Add Money menu."""
    if not is_admin(update.effective_user.id):
        return
    if len(context.args) != 1:
        await update.effective_message.reply_text(
            "❌ Format: /removepayment <amount>\nExample: /removepayment 500")
        return
    try:
    ...
    if amount not in BOHUDUR_PAYMENT_PRESETS:
        await update.effective_message.reply_text(
            "❌ Available fixed amounts: " + ", ".join(str(x) for x in BOHUDUR_PAYMENT_PRESETS))
        return
    if amount not in get_payment_presets():
        await update.effective_message.reply_text(f"ℹ️ {amount} TK already removed from Add Money menu.")
        return
    remove_payment_preset(amount)
    await update.effective_message.reply_text(
        f"✅ {amount} TK Add Money button removed.\n\n"
        "Custom Amount button will still remain.\n"
        "Use /addpayment <amount> to add it again.")


async def addpayment_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin command to restore a fixed Add Money amount."""
    if not is_admin(update.effective_user.id):
        return
    if len(context.args) != 1:
        await update.effective_message.reply_text(
            "❌ Format: /addpayment <amount>\nExample: /addpayment 500")
        return
    try:
        amount = int(context.args[0])
    except ValueError:
    ...
    if amount not in BOHUDUR_PAYMENT_PRESETS:
        await update.effective_message.reply_text(
            "❌ Available fixed amounts: " + ", ".join(str(x) for x in BOHUDUR_PAYMENT_PRESETS))
        return
    presets = list(get_payment_presets())
    if amount not in presets:
        presets.append(amount)
        presets.sort(key=BOHUDUR_PAYMENT_PRESETS.index)
        set_payment_presets(presets)
    await update.effective_message.reply_text(f"✅ {amount} TK Add Money button added back.")
...
async def main_async():
    ...
    application.add_handler(CommandHandler("removepayment", removepayment_cmd))
    application.add_handler(CommandHandler("addpayment", addpayment_cmd))
