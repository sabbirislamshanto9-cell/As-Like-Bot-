...
async def _send_page_message(message, text, reply_markup=None, entities=None):
    """Send a page while KEEPING the main reply keyboard visible.

    Page-specific controls are inline buttons; the normal main-menu buttons
    intentionally remain underneath so users can move directly to another
    section after opening any page.
    """
    kwargs = {}
    if entities:
    ...
    # Keep the main menu on private pages. Group chats do not get the private
    # reply keyboard.
    try:
        if getattr(getattr(message, "chat", None), "type", None) == "private":
            kwargs["reply_markup"] = build_main_menu()
    except Exception:
        logger.exception("Could not build persistent main menu for page")
    ...
    if reply_markup is not None:
        try:
            ...
            logger.exception("Could not attach inline page keyboard; page text remains usable")
    return sent
