...
async def _send_page_message(message, text, reply_markup=None, entities=None):
    """Send a page and remove the persistent main reply keyboard in the same message."""
...
async def reply_editable_content(message, key, reply_markup=None, **values):
    """Send an editable content item with Premium emoji and no persistent main keyboard."""
    ...
    if PREMIUM_STAR_HTML in text:
        return await _send_page_message(message, text, reply_markup=reply_markup)
...
async def menu_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if menu_is("balance"):
        ...
        page_text, page_entities, _ = add_premium_page_emoji("📜 ORDER HISTORY — SELECT A PAGE", [])
